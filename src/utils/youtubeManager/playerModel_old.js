...
export class YoutubePlayer {
    ...
    removeSongFromPlaylist(song, playlistId) {
        ...
        if (!playlistId || (playlistId && this.playlist.id === playlistId)) {
            let originalSongCount = this.playlist.songs.length;
            this.playlist.songs = this.playlist.songs.filter(songId => songId !== song.id);
            let songCountDiff = originalSongCount - this.playlist.songs.length;
            if (songCountDiff) this.playSongAt(this.currentVideo - songCountDiff);
        }
    }
